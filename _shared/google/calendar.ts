...
export const initOrgCalendar = async (orgId : number) => {
    const { data: orgData, error: orgFetchError } = await supabaseAdmin.from('organizations')
        .select(`*`)
        .eq('id', orgId)
        .limit(1)
        .single()
...
export async function createCalendarEvent(
    calendarId: string, 
    event : { name : string, description : string, start : string, end : string, location : string, source: { title: string, url: string } }
) {
