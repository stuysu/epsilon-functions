...
Deno.serve(async (req : Request) => {
    ...
    /* UPDATE ORG */
    const { data: orgData, error: updateError } = await supabaseClient.from('organizations')
    ...
    /* Try deleting edit */
    const { error: deleteEditError } = await supabaseClient.from('organizationedits')
        .delete()
        .eq('id', edit_id);
...
`
    ...
    if (deleteEditError) {
        return new Response("Failed to delete pre-existing edit. Please contact it@stuysu.org as soon as possible.", { status: 500 });
    }
