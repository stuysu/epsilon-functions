...
Deno.serve(async (req : Request) => {
    ...
    supabaseClient.from("memberships")
        ...
        .then(resp => {
            ...
            const startTime = datetime(createMeetingData[0].start_time).toZonedTime("America/New_York").format("MMMM d, YYYY, h:mm a");
            const endTime = datetime(createMeetingData[0].end_time).toZonedTime("America/New_York").format("MMMM d, YYYY, h:mm a");
