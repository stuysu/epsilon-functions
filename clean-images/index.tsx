...
Deno.serve(async (request: Request) => {
	...
	if (verifiedUser.permission !== 'ADMIN') {
	...
	const picture_ids = pictures.map((picture) => {
		const array = picture.picture.split('/');
	...
	for (const f in folders) {
		...
		const active_pictures = picture_ids.filter(
			(picture) => picture[0] === folder.name
		);
		for (const i in data) {
			if (active_pictures.every((picture) => picture[1] !== data[i].name)) {
				delete_pictures.push(`org-pictures/${folder.name}/${data[i].name}`);
			}
		}
	}
