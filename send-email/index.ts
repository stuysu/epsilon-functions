...
Deno.serve(async (req: Request) => {
    ...
    if (
        !recipient || !content || !recipient_type || !recipient_address ||
        !content_type || !content_title || !content_body
    ) {
    ...
    if (permissionsError) {
        return new Response('Failed to fetch user permissions.', {
            status: 500,
        });
    }
    ...
    if (recipient_type === 'INDIVIDUAL') {
        const { data: validRecipient, error: validRecipientError } =
        ...
        if (validRecipientError) {
            return new Response('Failed to fetch recipient user.', {
                status: 500,
            });
        }

        if (!validRecipient || !validRecipient.length) {
            return new Response('Recipient is not a valid user.', {
                status: 401,
            });
        }
    }
    ...
    if (!isAdmin && content_type === 'CUSTOM') {
        return new Response('Invalid permissions for custom emails.', {
            status: 401,
        });
    }
    ...
    if (!isAdmin) {
        ...
        if (adminOrgsError) {
            return new Response('Failed to fetch user organizations.', {
                status: 500,
            });
        }
        ...
        if (recipient_type === 'ORGANIZATION') {
            ...
            if (!adminOrgIds.includes(Number(recipient_address))) {
                return new Response('Invalid permissions for sending emails.', {
                    status: 401,
                });
            }
        } else {
            ...
            const { data: recipientMemberships, error: rmError } =
                await supabaseClient
                    .from('memberships')
                    .select(`
                    id,
    ...
    if (recipient_type === 'INDIVIDUAL') {
        ...
        if (orgMembersError) {
            return new Response('Failed to fetch organization members.', {
                status: 500,
            });
        }
        ...
        (orgMembers).map((member: ct) => {
            if (
                member.active && member.users.active && !member.users.is_faculty
            ) recipients.push(member.users.email);
        });
    }
