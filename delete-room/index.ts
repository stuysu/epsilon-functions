    ...
    await supabaseClient.from('meetings')
        ...
        .then(async ({ data: meetings, error: meetingFetchError }) => {
            ...
            for (const meeting of meetings) {
                ...
                const { data: admins, error: adminFetchError } = await supabaseClient.from('memberships')
                    .select(`
                        users!inner (
                            first_name,
                            email
            ...
            /* update room */
            const { error: roomDeleteError } = await supabaseClient.from('rooms')
                .delete()
                .eq('id', room_id);
...
${value.removedMeetings.map(meeting => `${meeting.title} at ${datetime(meeting.start_time).toZonedTime("America/New_York").format("MMMM d, YYYY, h:mm a")}`).join("\n")}

This is due to the fact that the room they were initially held has now been removed from the service.
